//! API tests for the one-off payment *refund* webhook endpoint.
//!
...
async fn post_webhook_unhandled_event_type_returns_200_ignored() {
    let s = bogus_db_server().await;
    // A charge.succeeded event is known-but-unhandled here; ignored before the DB.
    let payload = event_payload(
        "evt_other",
        "charge.succeeded",
...
async fn post_webhook_returns_200_before_email_side_effect_completes() {
    ...
    let payload = event_payload(
        "evt_mailfail",
        "charge.refunded",
        charge_object("repo-mailfail", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
