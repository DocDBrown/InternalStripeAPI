//! API tests for the one-off payment checkout *expired* webhook endpoint.
//!
//! Assert the HTTP envelope (status codes) over a real bound axum server. Cases that
//! fail before the database is touched use a lazily-connected pool pointed at a closed
//! port and need no container; only persisting cases provision a real Postgres and skip
...
async fn post_webhook_db_unavailable_returns_500() {
    ...
    let payload = event_payload(
        "evt_dbfail",
        "checkout.session.expired",
        checkout_session_object("repo-dbfail"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
