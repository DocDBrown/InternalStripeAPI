//! API tests for the usage subscription activation webhook endpoint.
//!
...
/// (ensure_schema now provisions the canonical org_subscription_entitlements shape —
/// includes trial_end_at, updated_at, and the status CHECK — but the HTTP-envelope
/// assertions here are unaffected by the column set.)
...
async fn post_webhook_returns_200_before_email_side_effect_completes() {
    ...
    let payload = event_payload(
        "evt_mailfail",
        "customer.subscription.created",
        subscription_object("org-mailfail", "owner@example.test", "active"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
