//! Integration tests for the "Ensure Stripe customer mapping" workflow.
//!
...
//!   over its `database_url`; the test keeps its own pool to seed/inspect the mapping).
//! - Stripe -> wiremock (an unowned external; never a real account). The mock returns a
//!   schema-valid customer and records outbound requests so we can assert what was sent.
//!
//! The SUT reads PUBSUB_EMULATOR_HOST internally, which is process-global, so tests are
//! serialized through TEST_LOCK and each one stands up its own emulator + postgres +
//! mock, torn down on Harness drop. Capability gate: if the container runtime is
//! unreachable, setup() returns None and the test skips cleanly.
...
use internal_stripe_api::stripe_portal::ensure_customer_mapping::{Config, ensure_schema, run};
...
const CUSTOMERS_PATH: &str = "/v1/customers";
const MOCK_CUSTOMER_ID: &str = "cus_test_new";
...
/// A schema-valid customer body (only the required fields). Keeps the SUT on its success
/// path with the given id.
fn customer_body(customer_id: &str) -> Vec<u8> {
    ...
    .expect("serialize canned customer")
}

async fn mount_customer_success(server: &MockServer, customer_id: &str) {
    Mock::given(method("POST"))
        .and(path(CUSTOMERS_PATH))
        .respond_with(
            ResponseTemplate::new(200).set_body_raw(customer_body(customer_id), "application/json"),
        )
        .mount(server)
        .await;
}

async fn mount_customer_error(server: &MockServer) {
    let body = serde_json::to_vec(&json!({
        "error": { "type": "api_error", "message": "Customer creation failed." }
    }))
...
async fn customer_request_count(server: &MockServer) -> usize {
    server
        .received_requests()
        .await
        .unwrap_or_default()
        .into_iter()
        .filter(|r| r.url.path() == CUSTOMERS_PATH)
        .count()
}
...
async fn setup(suffix: &str) -> Option<Harness> {
    ...
    let req_topic_id = format!("itest-custmap-requested-{suffix}");
    let req_sub_id = format!("itest-custmap-worker-{suffix}");
    let reply_topic_id = format!("itest-custmap-provisioned-{suffix}");
    let reply_sub_id = format!("itest-custmap-provisioned-sub-{suffix}");
...
async fn mapping_count(db: &PgPool, org_id: &str) -> i64 {
    sqlx::query_scalar::<_, i64>("SELECT count(*) FROM stripe_customer_mappings WHERE org_id = $1")
        .bind(org_id)
        .fetch_one(db)
        .await
...
async fn mapping_customer_id(db: &PgPool, org_id: &str) -> Option<String> {
    ...
    .fetch_optional(db)
    .await
    .expect("select mapping")
}

async fn seed_mapping(db: &PgPool, org_id: &str, customer_id: &str) {
    sqlx::query("INSERT INTO stripe_customer_mappings (org_id, customer_id) VALUES ($1, $2)")
        .bind(org_id)
        .bind(customer_id)
        .execute(db)
        .await
...
async fn ensures_customer_mapping_from_pubsub_request_and_publishes_id() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    mount_customer_success(&h.mock, MOCK_CUSTOMER_ID).await;

    let request_id = format!("req-{}", unique_suffix());
    let org_id = format!("org-{}", unique_suffix());
    let body = request_body(json!({ "request_id": request_id, "org_id": org_id }));
    ...
    assert_eq!(resp["status"], "ensured");
    assert_eq!(resp["customer_id"], MOCK_CUSTOMER_ID);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn consumes_customer_ensure_request_from_request_subscription() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn acks_customer_ensure_request_after_successful_handling() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn returns_existing_customer_id_when_mapping_exists() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    let org_id = format!("org-{}", unique_suffix());
    seed_mapping(&h.db, &org_id, "cus_preexisting").await;
    ...
    assert_eq!(resp["status"], "ensured");
    assert_eq!(resp["customer_id"], "cus_preexisting");
    assert_eq!(resp["created"], false);

    // Step 2 short-circuits: Stripe must not have been called.
    assert_eq!(customer_request_count(&h.mock).await, 0);

    h.shutdown().await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn creates_stripe_customer_when_no_mapping_exists() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    let (resp, _attrs) = wait_for_response(&h.reply_sub, &request_id, Duration::from_secs(30))
        ...
        .expect("expected a response");
    assert_eq!(resp["created"], true);
    assert_eq!(customer_request_count(&h.mock).await, 1);

    h.shutdown().await;
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn persists_mapping_in_neon_postgres() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    assert_eq!(
        mapping_customer_id(&h.db, &org_id).await.as_deref(),
        Some(MOCK_CUSTOMER_ID)
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn rejects_request_missing_org_id() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    assert!(resp["customer_id"].is_null());
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn publishes_customer_id_to_reply_topic() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
...
async fn publishes_to_per_request_reply_topic_override() {
    ...
    let override_topic_id = format!("itest-custmap-override-{suffix}");
    let override_sub_id = format!("itest-custmap-override-sub-{suffix}");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn is_idempotent_across_duplicate_requests() {
    let Some(h) = setup(&unique_suffix()).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    // First ensure: creates the customer and persists the mapping.
    let first_id = format!("req-{}", unique_suffix());
    let body = request_body(json!({ "request_id": first_id, "org_id": org_id }));
    publish(&h.req_topic, body, HashMap::new()).await;
    ...
    // Second ensure for the same org: reuses the mapping, no second Stripe call.
    let second_id = format!("req-{}", unique_suffix());
    let body = request_body(json!({ "request_id": second_id, "org_id": org_id }));
    publish(&h.req_topic, body, HashMap::new()).await;
    ...
    assert_eq!(customer_request_count(&h.mock).await, 1);
    assert_eq!(mapping_count(&h.db, &org_id).await, 1);
...
async fn publishes_failed_response_when_stripe_errors() {
    ...
    mount_customer_error(&h.mock).await;
    ...
    // The Stripe create was attempted, but no mapping was persisted.
    assert_eq!(mapping_count(&h.db, &org_id).await, 0);
