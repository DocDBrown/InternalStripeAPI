//! Integration tests for the "Process usage subscription cancellation webhook".
//!
...
use internal_stripe_api::stripe_subscriptions::process_subscription_cancellation_webhook::{
...
fn subscription_object(org_id: &str, email: &str, status: &str) -> Value {
    json!({
        ...
        "items": {
            ...
            "data": [],
            "has_more": false,
            "url": "/v1/subscription_items?subscription=sub_test_cancellation"
        },
        "livemode": false,
...
async fn processes_valid_cancellation_webhook_marks_canceled_and_sends_email() {
    ...
    let payload = event_payload(
        ...
        subscription_object("org-cancel", "owner@example.test", "canceled"),
    );
...
async fn receives_stripe_webhook_payload() {
    ...
    let payload = event_payload(
        "evt_recv",
        "customer.subscription.deleted",
        subscription_object("org-recv", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
async fn rejects_webhook_with_invalid_signature() {
    ...
    let payload = event_payload(
        "evt_bad",
        "customer.subscription.deleted",
        subscription_object("org-bad", "owner@example.test", "canceled"),
    );
...
async fn rejects_webhook_with_missing_signature_header() {
    ...
    let payload = event_payload(
        "evt_nosig",
        "customer.subscription.deleted",
        subscription_object("org-nosig", "owner@example.test", "canceled"),
    );
...
async fn accepts_webhook_with_valid_signature() {
    ...
    let payload = event_payload(
        "evt_ok",
        "customer.subscription.deleted",
        subscription_object("org-ok", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn marks_entitlement_inactive_in_neon_postgres() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    seed_status(&h.db, "org-inactive", "active").await;

    let payload = event_payload(
        "evt_inactive",
        "customer.subscription.deleted",
        subscription_object("org-inactive", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
    ...
    assert_eq!(resp.status().as_u16(), 200);
    assert_eq!(
        entitlement_status(&h.db, "org-inactive").await.as_deref(),
        Some("canceled")
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn cancellation_is_idempotent_on_duplicate_event() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    let payload = event_payload(
        "evt_dup",
        "customer.subscription.deleted",
        subscription_object("org-dup", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn overrides_active_entitlement_to_canceled() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    // Cancellation is terminal: an active entitlement is overridden to canceled.
    seed_status(&h.db, "org-override", "active").await;

    let payload = event_payload(
        "evt_override",
        "customer.subscription.deleted",
        subscription_object("org-override", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
    ...
    assert_eq!(resp.status().as_u16(), 200);
    assert_eq!(
        entitlement_status(&h.db, "org-override").await.as_deref(),
        Some("canceled")
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn triggers_cancellation_email_via_mailersend() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    let payload = event_payload(
        "evt_mail",
        "customer.subscription.deleted",
        subscription_object("org-mail", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn cancellation_email_addressed_to_subscriber() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    let payload = event_payload(
        "evt_addr",
        "customer.subscription.deleted",
        subscription_object("org-addr", "subscriber@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
async fn does_not_send_email_when_update_fails() {
    // No schema -> the cancel write fails -> handler returns 500, no email.
    ...
    let payload = event_payload(
        "evt_dbfail",
        "customer.subscription.deleted",
        subscription_object("org-dbfail", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
    ...
    assert!(
        mailer_payloads(&h.mailer).await.is_empty(),
        "no cancellation email should be sent when the DB write fails"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn ignores_non_subscription_deleted_event_types() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    let payload = event_payload(
        ...
        subscription_object("org-other", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);

    let resp = post_webhook(&h.base_url, Some(&sig), &payload).await;
    assert_eq!(resp.status().as_u16(), 200);
    // Untouched: the non-deleted event leaves the active status intact, sends no email.
    assert_eq!(
