//! API tests for the dispute-created webhook endpoint.
//!
...
async fn post_webhook_unhandled_event_type_returns_200_ignored() {
    let s = bogus_db_server().await;
    // A dispute.updated event is known-but-unhandled here; ignored before the DB.
    let payload = event_payload(
        "evt_other",
