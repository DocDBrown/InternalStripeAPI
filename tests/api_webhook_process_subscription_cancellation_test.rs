//! API tests for the usage subscription cancellation webhook endpoint.
//!
...
async fn post_webhook_unhandled_event_type_returns_200_ignored() {
    let s = bogus_db_server().await;
    // A subscription.updated event is known-but-unhandled here; ignored before the DB.
    let payload = event_payload(
        "evt_other",
...
async fn post_webhook_returns_200_before_email_side_effect_completes() {
    ...
    let payload = event_payload(
        "evt_mailfail",
        "customer.subscription.deleted",
        subscription_object("org-mailfail", "owner@example.test", "canceled"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
