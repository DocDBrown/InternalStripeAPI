//! Integration tests for the "Process usage subscription renewal payment failed webhook".
//!
...
use internal_stripe_api::stripe_subscriptions::process_subscription_renewal_failed_webhook::{
...
/// A schema-valid `Invoice` (the required fields plus the optionals the handler reads).
/// Nested required structs collapse to `{}`/defaults.
...
async fn processes_valid_renewal_failed_webhook_marks_past_due_and_sends_email() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    // The subscription was active; its renewal payment now fails.
    seed_status(&h.db, "org-fail", "active").await;

    let payload = event_payload(
        "evt_fail",
        "invoice.payment_failed",
        invoice_object("in_fail", "org-fail", "owner@example.test"),
    );
    ...
    assert_eq!(
        entitlement_status(&h.db, "org-fail").await.as_deref(),
        Some("past_due")
    );
...
async fn rejects_webhook_with_invalid_signature() {
    ...
    // Untouched: still active, no email.
    assert_eq!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn marks_entitlement_past_due_in_neon_postgres() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    seed_status(&h.db, "org-pd", "active").await;

    let payload = event_payload(
        "evt_pd",
        "invoice.payment_failed",
        invoice_object("in_pd", "org-pd", "owner@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
    ...
    assert_eq!(resp.status().as_u16(), 200);
    assert_eq!(
        entitlement_status(&h.db, "org-pd").await.as_deref(),
        Some("past_due")
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn past_due_is_idempotent_on_duplicate_event() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    // Only one dunning email despite the duplicate delivery.
    assert_eq!(mailer_payloads(&h.mailer).await.len(), 1);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn does_not_resurrect_canceled_entitlement() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    // A canceled subscription must not be flipped back to past_due by a late failure.
    seed_status(&h.db, "org-canceled", "canceled").await;

    let payload = event_payload(
        "evt_canceled",
        "invoice.payment_failed",
        invoice_object("in_canceled", "org-canceled", "owner@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
    ...
    assert_eq!(resp.status().as_u16(), 200);
    assert_eq!(
        entitlement_status(&h.db, "org-canceled").await.as_deref(),
        Some("canceled")
    );
    assert!(mailer_payloads(&h.mailer).await.is_empty());
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn triggers_dunning_email_via_mailersend() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    seed_status(&h.db, "org-mail", "active").await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn dunning_email_addressed_to_invoice_customer() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    seed_status(&h.db, "org-addr", "active").await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn does_not_send_email_when_update_fails() {
    // No schema -> the update fails -> handler returns 500, no email.
    ...
    assert!(
        mailer_payloads(&h.mailer).await.is_empty(),
        "no dunning email should be sent when the DB write fails"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn ignores_non_invoice_payment_failed_event_types() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    ...
    // An invoice.paid event (same Invoice object) is parsed and ignored here.
    let payload = event_payload(
        "evt_other",
        "invoice.paid",
    ...
    let resp = post_webhook(&h.base_url, Some(&sig), &payload).await;
    assert_eq!(resp.status().as_u16(), 200);
    // Untouched: a non-failure event leaves the entitlement active and sends no email.
    assert_eq!(
