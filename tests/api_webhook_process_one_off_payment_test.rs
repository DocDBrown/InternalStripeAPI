//! API tests for the Stripe one-off payment webhook endpoint.
//!
//! These assert the HTTP envelope (status codes) over a real bound axum server.
//! Most cases fail before the database is touched, so they use a lazily-connected
//! pool pointed at a closed port and need no container. Only the cases that must
//! persist (valid/duplicate/email) provision a real Postgres via testcontainers and
//! a wiremock MailerSend; those skip cleanly when the container runtime is absent.
...
/// Server whose DB points at a closed port: anything reaching persistence fails fast.
...
/// Server backed by a real Postgres + a MailerSend mock returning `mailer_status`.
...
async fn post_webhook_malformed_json_body_returns_400() {
    let s = bogus_db_server().await;
    // Signed so the signature passes; parsing then fails -> 400.
...
async fn post_webhook_unhandled_event_type_returns_200_ignored() {
    let s = bogus_db_server().await;
    // Known-but-unhandled type; ignored before the database is touched.
    let payload = event_payload(
        "evt_other",
...
async fn post_webhook_returns_200_before_email_side_effect_completes() {
    // MailerSend returns 500; the webhook must still return 200 (email is not a
    // precondition of the response).
...
async fn post_webhook_empty_body_returns_400() {
    let s = bogus_db_server().await;
    // Signed empty body: signature passes, parsing the empty payload fails -> 400.
    let body = String::new();
    let sig = Webhook::generate_test_header(&body, WEBHOOK_SECRET, None);
