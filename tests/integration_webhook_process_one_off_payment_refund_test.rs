//! Integration tests for the "Process one-off payment refunded webhook".
//!
...
use internal_stripe_api::stripe_payments::process_one_off_payment_refund_webhook::{
...
/// A schema-valid refunded `Charge` (only the required fields the client needs to
/// deserialize), plus metadata and billing email the handler reads.
fn charge_object(repository_id: &str, email: &str) -> Value {
    json!({
        "id": "ch_test_refund",
        "object": "charge",
        "amount": 5000,
...
async fn processes_valid_refund_webhook_revokes_and_sends_email() {
    ...
    let payload = event_payload(
        "evt_refund",
        "charge.refunded",
        charge_object("repo-refund", "buyer@example.test"),
    );
    ...
    assert_eq!(
        entitlement_status(&h.db, "repo-refund").await.as_deref(),
        Some("refunded")
    );
...
async fn receives_stripe_webhook_payload() {
    ...
    let payload = event_payload(
        "evt_recv",
        "charge.refunded",
        charge_object("repo-recv", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
async fn rejects_webhook_with_invalid_signature() {
    ...
    let payload = event_payload(
        "evt_bad",
        "charge.refunded",
        charge_object("repo-bad", "buyer@example.test"),
    );
...
async fn rejects_webhook_with_missing_signature_header() {
    ...
    let payload = event_payload(
        "evt_nosig",
        "charge.refunded",
        charge_object("repo-nosig", "buyer@example.test"),
    );
...
async fn accepts_webhook_with_valid_signature() {
    ...
    let payload = event_payload(
        "evt_ok",
        "charge.refunded",
        charge_object("repo-ok", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
async fn revokes_entitlement_in_neon_postgres() {
    ...
    let payload = event_payload(
        "evt_revoke",
        "charge.refunded",
        charge_object("repo-revoke", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
async fn revokes_previously_paid_entitlement() {
    ...
    // A completed payment for this repo already exists; the refund must revoke it.
    seed_paid(&h.db, "repo-paid").await;

    let payload = event_payload(
        "evt_paid",
        "charge.refunded",
        charge_object("repo-paid", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);

    let resp = post_webhook(&h.base_url, Some(&sig), &payload).await;
    assert_eq!(resp.status().as_u16(), 200);
    // The refund DOES override paid -> refunded (access revoked).
    assert_eq!(
...
async fn revocation_is_idempotent_on_duplicate_event() {
    ...
    let payload = event_payload(
        "evt_dup",
        "charge.refunded",
        charge_object("repo-dup", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn triggers_refund_email_via_mailersend() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    let payload = event_payload(
        "evt_mail",
        "charge.refunded",
        charge_object("repo-mail", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn refund_email_addressed_to_charge_customer() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    let payload = event_payload(
        "evt_addr",
        "charge.refunded",
        charge_object("repo-addr", "customer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
async fn does_not_send_email_when_revocation_fails() {
    ...
    let payload = event_payload(
        "evt_dbfail",
        "charge.refunded",
        charge_object("repo-dbfail", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
    ...
    assert!(
        mailer_payloads(&h.mailer).await.is_empty(),
        "no refund email should be sent when the DB write fails"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn ignores_non_charge_refunded_event_types() {
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    // A charge.succeeded event (same Charge object) is parsed and ignored here.
    let payload = event_payload(
        "evt_other",
        "charge.succeeded",
        charge_object("repo-other", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn stamps_updated_at_on_refunded_transition() {
    // Verifies the canonical `updated_at` audit column is populated on the transition
    // to refunded (defaulted on insert, and set explicitly by the upsert's SET clause).
    let Some(h) = setup(true, 202).await else {
        eprintln!("skipping: container runtime unavailable");
        return;
    };
    let payload = event_payload(
        "evt_updated_at",
        "charge.refunded",
        charge_object("repo-updated-at", "buyer@example.test"),
    );
    let sig = Webhook::generate_test_header(&payload, WEBHOOK_SECRET, None);
    ...
    // The audit timestamp must be present on the refunded row.
    assert!(
        entitlement_updated_at_is_set(&h.db, "repo-updated-at").await,
        "updated_at should be populated on the refunded transition"
    );
