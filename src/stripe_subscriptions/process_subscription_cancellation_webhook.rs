//! src/stripe_subscriptions/process_subscription_cancellation_webhook.rs
//!
//! Workflow: **Process Stripe usage subscription cancellation webhook**
//!   1. Receive the Stripe usage subscription cancellation webhook (axum POST handler).
//!   2. Verify the Stripe usage subscription cancellation webhook signature.
//!   3. Update the org's usage subscription entitlement to inactive in Neon Postgres.
//!   4. Trigger the usage subscription cancellation email via MailerSend.
//!
//! The actionable event is `customer.subscription.deleted` (the inbound confirmation
//! that a subscription was canceled). Cancellation is terminal: it overrides any prior
//! status, is idempotent on replay, and the email is sent only on the first transition
//! to `canceled`.
//!
...
/// Route the cancellation webhook is delivered to.
pub const WEBHOOK_PATH: &str = "/webhooks/stripe/subscription-cancellation";
...
pub struct AppState {
    ...
    /// "from" address used on cancellation emails.
...
///     `unpaid`, `paused`). `canceled` (this file's written status) is in the set.
///
...
async fn handle_webhook(
    ...
    // Only subscription deletions are actionable; everything else is ignored.
    let subscription = match event.data.object {
        EventObject::CustomerSubscriptionDeleted(subscription) => subscription,
        other => {
            tracing::info!(?other, "ignoring non subscription-deleted event");
    ...
    // Step 3: mark the entitlement canceled/inactive (idempotent; terminal override).
    let newly_canceled = match cancel_entitlement(&state.db, &org_id, &subscription_id).await {
        Ok(newly_canceled) => newly_canceled,
        Err(e) => {
            tracing::error!(error = %e, org_id = %org_id, "failed to cancel entitlement");
            return StatusCode::INTERNAL_SERVER_ERROR;
        }
    };

    // Step 4: send the cancellation email only on the first transition to canceled.
    if newly_canceled {
        match subscription.metadata.get("email") {
            Some(email) => {
                if let Err(e) = send_cancellation_email(&state, &org_id, email).await {
            ...
            None => {
                tracing::warn!("no email in subscription metadata; skipping cancellation email")
            }
        }
    } else {
        tracing::info!(org_id = %org_id, "entitlement already canceled; no change");
    }

    StatusCode::OK
}

/// Mark the org's subscription entitlement `canceled`. Returns true only when this call
/// transitioned the row. Cancellation overrides any prior status; an already-`canceled`
/// row is a no-op (idempotent replay).
///
/// `updated_at` is stamped now() on both the insert and the update so the transition is
/// dated on every real state change. The guard `status <> 'canceled'` excludes an
/// already-canceled row (replay) from the UPDATE, so neither status nor updated_at is
    ...
    let row = sqlx::query(
        "INSERT INTO org_subscription_entitlements (org_id, status, subscription_id, activated_at, updated_at)
         ...
         DO UPDATE SET status = 'canceled',
                       ...
                       updated_at = now()
         WHERE org_subscription_entitlements.status <> 'canceled'
         RETURNING org_id",
    )
    .bind(org_id)
...
/// POST the cancellation notice to MailerSend's send-email endpoint.
async fn send_cancellation_email(
    ...
    let body = json!({
        ...
        "subject": "Your usage subscription has been canceled",
        "text": format!("Your usage subscription for {org_id} has been canceled. We're sorry to see you go."),
    });
    state
        .http
        ...
        .await?
        .error_for_status()?;
    tracing::info!(org_id = %org_id, "cancellation email dispatched via MailerSend");
    Ok(())
}